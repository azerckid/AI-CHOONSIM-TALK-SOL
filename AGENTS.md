...
- **App Directory (`/apps/web`)**: 실제 **구현(Implementation)** 영역이다. **본 프로젝트(AI-CHOONSIM-TALK-SOL)는 `apps/web/`을 App Directory로 사용한다.** 소스 코드(`app/`), 빌드 설정, 의존성(package.json, node_modules)을 `apps/web/`에서 관리한다.
...
| 카테고리 | 스킬명 | 주요 역할 |
| :--- | :--- | :--- |
| **운영 및 거버넌스** | `manage-collaboration` | AI-Human 협업 표준(Anti-Rush, 리뷰 등) 강제 |
...
| | `verify-docs` | 문서 레이어 정합성 및 네이밍/메타데이터 검증 |
| | `verify-drizzle-schema` | Drizzle 스키마 설계 정합성 및 기술 표준 검증 |
| | `verify-rag-logic` | RAG 프롬프트, 인용 규칙 및 벡터 검색 로직 검증 |
