...
## 5. 기밀 컴퓨팅(Confidential Computing) — Phase 3+ 비전

> 검토일: 2026-04-03
> 상태: 장기 로드맵 (해커톤 후 연구 과제)

### 배경 및 문제 의식

AI Companion 서비스 특성상 유저는 매우 개인적인 감정·비밀을 AI에게 털어놓습니다.
현재 채팅 데이터는 Turso DB + Better Auth 세션으로 보호되지만, 서버 레벨에서는 평문으로 처리됩니다.
"AI가 데이터를 처리하면서 동시에 기밀 상태를 유지"하는 기술이 솔라나 생태계에서 성숙해지고 있어 장기 도입을 검토합니다.

### 주요 기술 접근

#### 5-1. Secret Network Cross-chain TEE (가장 현실적)

- **방식**: 솔라나 지갑으로 서명하되, 채팅 데이터는 Intel SGX 기반 TEE 내부에서 처리
- **연동**: 솔라나 ↔ Secret Network 크로스체인 메시징 (IBC 브릿지 완료 상태)
- **장점**: 솔라나 속도·유동성 유지하면서 민감 데이터만 격리
- **한계**: AI 추론을 TEE 내부에서 실행하려면 외부 LLM API 의존 구조 변경 필요

#### 5-2. Arcium (MPC + TEE 네트워크)

- **방식**: 다자간 연산(MPC) 노드로 암호화된 데이터를 복호화 없이 연산
- **장점**: 소프트웨어 암호학적 기밀성 → 하드웨어 TEE 신뢰 의존도 낮춤
- **상태**: 솔라나 생태계 내 기밀 컴퓨팅 표준을 목표로 활발히 확장 중
- **한계**: 프로덕션 적용 사례 아직 초기 단계

#### 5-3. MagicBlock Private Ephemeral Rollups (실험적)

- **방식**: Intel TDX 보호 프라이빗 롤업에서 채팅 상태 처리 → 세션 종료 시 결과 증명만 솔라나에 머지
- **장점**: 빠른 실시간 상태 처리 + 외부 비공개
- **한계**: 아직 게임/실시간 시나리오 중심, 범용 적용 불확실

#### 5-4. 클라이언트 암호화 + Shadow Drive (단기 적용 가능)

- **방식**: 지갑 공개키(NaCl 비대칭 암호화)로 프론트엔드에서 채팅 데이터 암호화 → Shadow Drive/Arweave에 저장
- **장점**: 구현 난이도 낮음, 해당 지갑 소유자만 복호화 가능
- **한계**: AI가 암호화된 데이터를 추론할 수 없음 — "저장 기밀성"만 확보, "연산 기밀성" 불가
...
|--------|------|----------------|
| 전송 | HTTPS TLS | 동일 |
| 저장 | Turso DB (서버 평문) | 클라이언트 암호화 + Shadow Drive |
| 연산 | 서버 평문 처리 | Secret Network TEE 또는 Arcium MPC |
| 온체인 증명 | cNFT 기록 | 기밀 연산 결과 ZK 증명 |

### 권고 도입 순서

```
Phase 2: 클라이언트 암호화 + Shadow Drive (저장 기밀성 확보)
Phase 3: Secret Network Cross-chain TEE 파일럿 (연산 기밀성)
Phase 3+: Arcium MPC 통합 검토 (완전한 소프트웨어 기밀성)
```
