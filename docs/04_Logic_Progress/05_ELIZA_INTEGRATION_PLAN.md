...
**이유**: 현재 elizaOS 생태계가 이 프로젝트의 핵심 Solana 기술 스택을 지원하지 않음.

| 미지원 항목 | 내용 |
|-------------|------|
| SPL Token-2022 Transfer Fee | `@elizaos/plugin-solana` 미지원 → CHOCO 연동 불가 |
| cNFT / Bubblegum | 공식 플러그인 없음 → 기억 각인 기능 연동 불가 |
| elizaOS v2 안정성 | 알파 태그(v1.2.6), 플러그인 생태계 파편화 진행 중 |

**결정**: elizaOS가 Token-2022 + cNFT를 공식 지원하는 시점에 재착수.
그 전까지 기존 LangGraph 웹앱 채팅 경로를 유지.
