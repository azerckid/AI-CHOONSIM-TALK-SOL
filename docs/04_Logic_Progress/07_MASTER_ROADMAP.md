...
- [ ] `stream.ts`의 `model.stream()` 직접 호출을 `graph.stream()` 기반으로 교체
- [ ] `summarizeNode`가 스트리밍 경로에도 적용되도록 수정 → 대화 요약 메모리 정상 작동
- [ ] `createChatGraph()`를 모듈 수준에서 한 번만 컴파일 (매 요청 재컴파일 제거)
- [ ] `graph.ts` / `stream.ts` 코드 중복 제거 (시스템 프롬프트 빌드 로직 통합)
