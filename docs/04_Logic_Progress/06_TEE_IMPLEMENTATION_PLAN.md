...
> 작성일: 2026-04-11
> Updated: 2026-04-30
> 목표 완료: **보류 — 다음 Phase로 이관**
> 목적: 서버 관리자도 접근 불가한 격리된 환경에서 춘심 에이전트의 프라이빗 키를 관리 — "춘심이 지갑은 춘심이만 제어한다"는 신뢰를 온체인으로 증명

## ⚠️ 보류 결정 (2026-04-30)

**이유**: Lit Protocol 및 Solana TEE 생태계 미성숙.

| 미지원 항목 | 내용 |
|-------------|------|
| Solana 자체 TEE | 솔라나 L1은 TEE 비제공 — 항상 외부 솔루션 필요 |
| Datil 테스트넷 | **2026-02-25 종료** — 문서 기반 설계 경로 무효 |
| Ed25519 네이티브 PKP | Lit 자체 로드맵상 미완성 |
| `@lit-protocol/pkp-solana` | Wrapped Keys SDK로 대체됨 — 설계 전면 재작성 필요 |
| Token-2022 / cNFT 서명 | Wrapped Keys로도 레퍼런스 구현 없음 |

**결정**: Lit Protocol Naga 메인넷이 Ed25519 네이티브 PKP를 안정 지원하고,
Token-2022 트랜잭션 서명 레퍼런스가 나오는 시점에 재착수.
그 전까지 `SOLANA_AGENT_PRIVATE_KEY` 환경변수 방식 유지.
